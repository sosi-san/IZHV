...
public class Gun : MonoBehaviour
{
    ...
    /// <summary>
    /// Transform used spawn bullets at end of gun
    /// </summary>
    private Transform mGunMuzzle;
    ...
    private void Awake()
    {
        ...
        mGunMuzzle = transform.Find("Muzzle");
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            var basePosition = mGunMuzzle.position;
            var baseRotation = transform.rotation;
            var shotGunAngleRight = Quaternion.Euler(Vector3.up*shotgunSpread);
            var shotGunAngleLeft = Quaternion.Euler(Vector3.up*-shotgunSpread);
            
            SpawnBullet(basePosition, baseRotation * shotGunAngleLeft);
            SpawnBullet(basePosition, baseRotation);
            SpawnBullet(basePosition, baseRotation * shotGunAngleRight);
        }
        else
        {
            SpawnBullet(
